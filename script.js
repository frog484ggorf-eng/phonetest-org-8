...
function generateTips(score, answersArr){
  ...
  if(answersArr[3] >=3 || answersArr[9] >=3){
    // контроль времени и внимание
if(answersArr[3] >=3 || answersArr[9] >=3){
  tips.push({
    title: 'Контроль времени',
    text: 'Попробуйте использовать таймер или приложение для отслеживания экранного времени. Это поможет заметить, куда уходит время и легче регулировать использование телефона.'
  });
  tips.push({
    title: 'Фокус и внимание',
    text: 'Выделяйте конкретное время для отдыха с телефоном и отдельное — для дел. Чёткие границы помогают сохранять концентрацию и не отвлекаться по мелочам.'
  });
}
